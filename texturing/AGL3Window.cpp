...
void AGLWindow::WaitForFixedFPS(float frame_time) {
    double real_frame_time = glfwGetTime() - prev_time;
    double wait_time = 1/frame_time - real_frame_time;
    double FPS;
    if (wait_time > 0.0)
    {
        ...
        FPS = 1/real_frame_time;
    string newTitle = to_string(1000.0/FPS) + "ms / " + to_string(FPS) + " FPS. Points: " + to_string(points);
    glfwSetWindowTitle(window,newTitle.c_str());
    prev_time = glfwGetTime();
}
void AGLWindow::setCursor(int x, int y)
{
    glfwSetCursorPos(window, x, y);
}
void AGLWindow::setPoints(int newPoints)
{
    points = newPoints;
}
